...
childs = [10,20,30,40,50,60,70,80]
child_scores = []
for child in childs:
    clf = xgb.XGBClassifier(n_estimators = 90, max_depth = 5, silent = True, n_jobs = -1,min_child_weight=child,
                        booster='gbtree',random_state=7, subsample = 0.8, colsample_bytree = 0.8,
                        learning_rate=0.1, objective = 'binary:logistic',gamma=2)#scale_pos_gamma
    child_scores.append(cv_score(clf,x_train,y_train))
child_scores = np.array(child_scores)

np.savetxt('xgb_childs_10_80_0.1_90_5.txt', child_scores, delimiter=',')
#saveFigure(childs, gamma_scores, 'xgb_gamma_1_2_0.1_90_5')
